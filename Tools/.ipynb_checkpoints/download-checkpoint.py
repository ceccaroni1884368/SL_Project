...
geolocator = Nominatim(user_agent="SL_project")
...
def make_record(r):
    ...
    if item['status'] == 'ok':
        ...
        # City Name
        try:
            rows.append(geolocator.reverse(str(item['data']['city']['geo'][0])+', '+str(item['data']['city']['geo'][1])))
        except: 
            rows.append(np.nan)  
        # Latitude
        try:
        ...
        try:
            ...
            rows.append(np.nan)
        # Create record
        record = pd.Series(rows, index=['local date','local time', 'location', 'lat', 'lon', 'aqi'])
        # I AQI
        try:
